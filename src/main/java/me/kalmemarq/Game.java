...
public class Game implements Runnable {
    ...
    private static final LongSupplier timeNs = () -> System.nanoTime() / 1_000_000L;
    ...
    public void run() {
        ...
        this.terrainShadow = new Shader("terrain_fog");
        ...
        long lastTime = timeNs.getAsLong();
        int frameCounter = 0;
        ...
        try {
            ...
            int tickCounter = 0;
            long prevTimeMillis = timeNs.getAsLong();
            int ticksPerSecond = 60;
            float tickDelta = 0;

            while (!this.window.shouldClose()) {
                long now = timeNs.getAsLong();
                ...
                while (timeNs.getAsLong() - lastTime > 1000L) {
                    lastTime += 1000L;
                    this.window.setTitle(frameCounter + " FPS " + tickCounter + " TPS C: " + WorldRenderer.chunksRendererPerFrame + "/" + this.worldRenderer.getChunkCount() + " x=" + String.format("%.3f", this.player.position.x) + ",y=" + String.format("%.4f", this.player.position.y) + ",z=" + String.format("%.3f", this.player.position.z));
                    frameCounter = 0;
                    tickCounter = 0;
                }
    ...
    private void render(float tickDelta) {
        ...
        this.terrainShadow.bind();
        this.terrainShadow.setUniform("uProjection", this.projection);
        this.terrainShadow.setUniform("uModelView", this.modelView);
        this.terrainShadow.setUniform("uColor", 1f, 1f, 1f, 1f);
        this.terrainShadow.setUniform("uFogDensity", 0.04f);
        this.terrainShadow.setUniform("uFogColor", 0.0f, 0.0f, 0.0f, 1f);
        this.terrainShadow.setUniform("uSampler0", 0);

        this.worldRenderer.render(this.terrainShadow, this.frustum,  1);
        ...
        if (this.blockHitResult != null) {
            ...
            this.selectionShader.setUniform("uColor", 1f, 1f, 1f, (float)Math.sin((double)timeNs.getAsLong() / 100.0d) * 0.2f + 0.4f);
